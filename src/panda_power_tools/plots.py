...
def plot_net(net, index=False, generic_coordinates=False, text_size: float = 0.05):
    ...
    collections_data['coords_buses'] = list(map(lambda coord: (coord[0]-0.4, coord[1]+0.03),
    ...
    if len(trafos_index_in_service):
        tr_in_service = plot.create_trafo_collection(net, trafos=trafos_index_in_service.index, size=0.05, linewidths=1,
        ...
        trafos_name = [f'{t.at["name"]}' for n, t in trafos_index_in_service.iterrows()]

        trafos_mark = [f'{t.at["std_type"]}' for n, t in trafos_index_in_service.iterrows()]
    ...
    if len(trafos_index_not_in_service):
        tr_not_in_service = plot.create_trafo_collection(net, trafos=trafos_index_not_in_service.index, size=0.05,
                                                         linewidths=1, linestyle='--', bus_geodata=bus_geodata)
    ...
    # вывод сборных шин
    bbc = plot.create_busbar_collection(net, buses=[1, 2, 26, 27], color='k', linewidth=3)
    ...
    lc = plot.create_line_collection(net, color='k', lines=lines_without_geo, linewidths=1, use_bus_geodata=True)
    lc_with_geo = plot.create_line_collection(net, lines=lines_with_geo, color='k', linewidths=1,
    ...
    collections_data['collections'].extend([lc, lc_with_geo, bbc, (switches, helper_lines)])
